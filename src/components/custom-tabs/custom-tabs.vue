  ...
  <view :class="['tab', { 'tab-fixed': fixed }]">
    ...
    >
      <view
        ...
        :id="`tab_${index}`"
        :style="[
          { padding: `0 ${tabSpacing}rpx` },
          tabIndex == index ? activeTextStyle : defaultTextStyle,
        ]"
        :key="index"
        @click="
          putChange(item.name, index);
          tabChange(index);
        "
      >
    ...
    <view
      ...
      :style="{
        ...
        transition: `transform ${d / 1000}s ease-in-out`,
      }"
    >
      <slot></slot>
    </view>
    ...
    uni.$on(this.watchTabValueKey, (o) => {
      ...
      if (typeof o.oldValue == "undefined") {
        ...
        this.timer = setTimeout(() => {
          if (this.tabList[this.value])
            this.putChange(this.tabList[this.value].name);
        }, 5);
      } else
        this.tabList.forEach((item) => {
          if (item.label == o.oldValue && item.name == o.name)
            item.label = o.newValue;
        });
    });
  },
  watch: {
